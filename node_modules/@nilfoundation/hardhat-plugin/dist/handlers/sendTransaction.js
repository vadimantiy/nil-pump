...
async function prepareDeployment(params, context) {
    const deployed = await context.wallet.deployContract({
        shardId: context.hre.config.shardId ?? (0, conversion_1.shardNumber)(context.wallet.address),
    ...
    const receipt = await (0, niljs_1.waitTillCompleted)(context.client, deployed.hash);
    if (context.debug) {
...
async function handleDirectTransaction(params, context) {
    ...
    const receipt = await (0, niljs_1.waitTillCompleted)(context.client, hash);
    if (context.debug) {
